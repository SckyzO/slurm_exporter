...
func TestParseQueueMetrics(t *testing.T) {
	...
	// Running cores (c_running)
	assert.Equal(t, 12.0, qm.c_running["foo"]["15306588"])
	assert.Equal(t, 12.0, qm.c_running["bar"]["15452401"])
	...
	// Pending cores
	assert.Equal(t, 12.0, qm.c_pending["Licenses"]["bar"]["15452394"])

	// Suspended job and cores — verifies the c_suspended bug fix (was a copy-paste
	// that incremented suspended twice instead of populating c_suspended).
	assert.Equal(t, 1.0, qm.suspended["bar"]["15452466"], "suspended job count")
	assert.Equal(t, 12.0, qm.c_suspended["bar"]["15452466"], "suspended core count must be populated (regression for copy-paste bug fix)")

	// Other states
	assert.Equal(t, 1.0, qm.cancelled["bar"]["15452465"])
	assert.Equal(t, 12.0, qm.c_cancelled["bar"]["15452465"])
	...
	assert.Equal(t, 1.0, qm.node_fail["bar"]["15452441"])
